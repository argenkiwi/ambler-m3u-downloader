...
export function promptOptions(dependencies: {
  ...
  return async (state: State): Promise<Next<State> | null> => {
    ...
    const options: { name: string; value: Next<State> | null }[] = [
      ...
      { name: "list", value: new Next(dependencies.onList, state) },
    ];

    if (hasKhinsiderUrls) {
      options.push({ name: "resolve", value: new Next(dependencies.onResolve, state) });
    } else {
      options.push({ name: "download", value: new Next(dependencies.onDownload, state) });
    }
